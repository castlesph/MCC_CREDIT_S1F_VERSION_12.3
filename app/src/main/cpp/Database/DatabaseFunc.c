...
int inMMTReadRecord(int inHDTid,int inMITid)
{
	int result;
	int inResult = d_NO;
	char *sql = "SELECT MMTid, szHostName, HDTid, szMerchantName, MITid, szTID, szMID, szATCMD1, szATCMD2, szATCMD3, szATCMD4, szATCMD5, fMustSettFlag, szBatchNo, szRctHdr1,szRctHdr2, szRctHdr3, szRctHdr4, szRctHdr5, szRctFoot1, szRctFoot2, szRctFoot3, fMMTEnable, szCurrency, fPasswordEnable, szPassword, inFailedREV, szMKEPassword FROM MMT WHERE HDTid = ? AND MITid = ? AND fMMTEnable = ? ORDER BY MMTid";
	int incount = 0;
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
            ...
            /*szMKEPassword*/
            strcpy((char*)strMMT[incount].szMKEPassword, (char *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
		}
	} while (result == SQLITE_ROW);
...
//    	CTOS_PrinterPutString(strTCT.szSuperPW);
  ...
  //  	CTOS_PrinterPutString(strTCT.szSystemPW);
    ...
    //	CTOS_PrinterPutString(strTCT.szEngineerPW);
		...
		/* szMKEPassword */
		//#1 added to fix - Issue is MKE PASSWORD is having wrong password when first transaction done is 
		...
		strcpy((char*)strTCT.szMKEPassword, (char *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
		vdDebug_LogPrintf("strTCT.szMKEPassword = [%s]", strTCT.szMKEPassword);
...
{
	int result;
	char *sql = "UPDATE TCT SET byTerminalType = ?, inMenuid = ?, inMenuidOldHost = ?, inMenuidNewHost = ?, inMenuidNHProfileMenu = ?, fRegister = ?, byTerminalModel = ?, inIdleMode = ? WHERE  TCTid = ?";
...
int inDatabase_InvoiceNumSearch(TRANS_DATA_TABLE *transData, char *hexInvoiceNo)
{
	...
	char *sql = "SELECT HDTid, szDate, szTime FROM TransData WHERE szInvoiceNo = ?";
    ...
    DebugAddHEX("transData->szDate", transData->szDate, 3);
	DebugAddHEX("transData->szTime", transData->szTime, 4);
...
int inCPT_SSLUpdate(int inSSLEnable)
{
	int result;
    //char *sql = "UPDATE CPT SET inCommunicationMode = ?, inIPHeader = ?";
    char *sql = "UPDATE CPT SET fSSLEnable = ? WHERE fIpUpdate = 1";
    ...
    inStmtSeq = 0;
    result = sqlite3_bind_int(stmt, inStmtSeq +=1, inSSLEnable);
	//result = sqlite3_bind_int(stmt, inStmtSeq +=1, inIPHeader);
...
int inReadDMTrxAppMenu2(int inIdleSaleType)
{
	...
	if (strTCT.inMenuid > 99)
		sprintf(szMenuFilename, "/data/data/com.Source.S1_MCC.MCC/DYNAMICMENU%03d.S3DB", strTCT.inMenuid);
	else
		sprintf(szMenuFilename, "/data/data/com.Source.S1_MCC.MCC/DYNAMICMENU%02d.S3DB", strTCT.inMenuid);
...
int inReadBaseDMTrxMenu(int *inNumRecs) {
    ...
    memset(szMenuFilename, 0x00, sizeof(szMenuFilename));
    if (strTCT.inMenuid > 99)
        sprintf(szMenuFilename, "/data/data/com.Source.S1_MCC.MCC/DYNAMICMENU%03d.S3DB",
                strTCT.inMenuid);
    else
        sprintf(szMenuFilename, "/data/data/com.Source.S1_MCC.MCC/DYNAMICMENU%02d.S3DB",
                strTCT.inMenuid);
...
int inReadDMTrx(int inSaleType)
{
	...
	do
	{
		...
		if(inResult == SQLITE_ROW)
		{
			...
			vdDebug_LogPrintf("1. inCtr: %d", inCtr);
    ...
    if(inSaleType == 4) /*add Login tranasction in PHQR menu*/
    {
		vdDebug_LogPrintf("2. inCtr: %d", inCtr);
		/* szButtonTitleLabel */
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemLabel, "LOGIN");

		/* szButtonItemFunc */
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemFunc, "inPHQRLogin");
	...
	{
	    /*
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemLabel, "CLEAR BATCH");
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemFunc, "vdCTOS_DeleteBatchPassword");
		++inCtr;	
		*/
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemLabel, "SET WIFI");
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemFunc, "vdCTOS_SetWIFI");
		++inCtr;			
	}
	else if(inSaleType == 0)
	{
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemLabel, "SET WIFI");
		strcpy((char*)strDynamicMenu[inCtr].szButtonItemFunc, "vdCTOS_SetWIFI");
		++inCtr;		
...
int inWaveGetValue(char *szFlag, char *szTable)
{
	int result;
	int inResult = d_NO;
	char *sql;
	...
	vdDebug_LogPrintf("*****szWaveGetValue*****");
	memset(szTempSQL, 0, sizeof(szTempSQL));
	sql=&szTempSQL;
	
	sprintf(szTempSQL, "select szValue from %s where szTag='%s'", szTable, szFlag);
	vdDebug_LogPrintf("sql command:%s", szTempSQL);
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			/*fPrintMercReceipt*/
			//fValue = fGetBoolean((BYTE *)sqlite3_column_text(stmt,inStmtSeq));    
            strcpy((char*)srTransRec.szWaveValue, (char *)sqlite3_column_text(stmt,inStmtSeq));

			fValue=VS_TRUE;
   		}
	} while (result == SQLITE_ROW);	   

    vdDebug_LogPrintf("szWaveGetValue Flag[%s] value[%s]", szFlag, srTransRec.szWaveValue);
	sqlite3_exec(db,"commit;",NULL,NULL,NULL);
...
int inMMTSaveEx2(int inSeekCnt)
{
	int result;
	char *sql = "UPDATE MMT SET szMerchantName = ?, szPassword = ?, fPasswordEnable = ?, szRctHdr1 = ?, szRctHdr2 = ?, szRctHdr3 = ?, szRctHdr4 = ?, szRctHdr5 = ?, szRctFoot1 =?, szRctFoot2 =?, szRctFoot3 =? WHERE MITid = ?";

    vdDebug_LogPrintf("inMMTSaveEx2 %d:", inSeekCnt);
    ...
    /* fPasswordEnable */
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strMMT[0].fPasswordEnable);
...
int inCDTReadMultiNew(char *szPAN, int *inFindRecordNum)
{
	int result;
	int inResult = d_NO;
	char *sql = "SELECT CDTid FROM CDT WHERE ? BETWEEN szPANLo AND szPANHi AND fCDTEnable = ?";
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			strMCDT[inCount].CDTid = sqlite3_column_int(stmt,inStmtSeq);	
