...
int inCTOS_WaveFlowProcess(void)
{
	...
	*/
    ...
    vdDebug_LogPrintf("1.inCTOS_GetCardFieldsCtls");
  ...
  if(strTCT.fEMVOnlinePIN == TRUE && srTransRec.fEMVPINEntered == FALSE)
  {
	...
	vdDebug_LogPrintf("1.0.BUILD&SEND START");
    inRet = inBuildAndSendIsoData();
	vdDebug_LogPrintf("2.BUILD&SEND END");
    if(d_OK != inRet)
    {
	...
	vdDebug_LogPrintf("1.inCTOS_SaveBatchTxn");
    ...
    vdDebug_LogPrintf("2.inCTOS_SaveBatchTxn");
