...
//	CTOS_PrinterPutString("inCTOSS_CLMInitTransaction");
	...
	sprintf(sztemp,"$$$$$::[%s],[%s],[%s],[%s],[%s]", szAmount,szOtherAmt,szTransType,szCatgCode,szCurrCode);
...
//	CTOS_PrinterPutString("inCTOSS_CLMVisaGetCapability");
	...
	if(rtn != 0xA0000004)
	{
//	CTOS_PrinterPutString("inCTOSS_CLMVisaGetCapability-1");
	
		vdDebug_LogPrintf("Get Capability Fail");
	}
	else
	{
//	CTOS_PrinterPutString("inCTOSS_CLMVisaGetCapability-2");
...
//    CTOS_PrinterPutString("vdCTOSS_CLMSetAIDAndEMVTag");
	...
	if (inNumRecord > 0)
	{
//    CTOS_PrinterPutString("vdCTOSS_CLMSetAIDAndEMVTag-1");
...
//    CTOS_PrinterPutString("vdCTOSS_CLMSetAIDAndEMVTag-2");
...
//    CTOS_PrinterPutString("vdCTOSS_CLMSetAIDAndEMVTag-end");
...
void vdCTOSS_WaveGetEMVData(BYTE *baChipData, USHORT usChipDataLen)
{
	...
	vdDebug_LogPrintf("--vdCTOSS_WaveGetEMVData--");
	...
	vdDebug_LogPrintf("TAG_9F26_EMV_AC | usLen[%d] | usChipDataLen[%d]", usLen, usChipDataLen);

	DebugAddHEX("1. baChipData",baChipData, 8);
	DebugAddHEX("1. srTransRec.stEMVinfo.T9F26",srTransRec.stEMVinfo.T9F26, 8);
...
//	TLVDataParse(stRCDataEx->baAdditionalData, stRCDataEx->usAdditionalDataLen);
	...
	vdDebug_LogPrintf("saturn inCTOSS_WaveAnalyzeTransaction");
	...
	else if(stRCDataAnalyze.usTransResult == d_EMV_CHIP_OFF_APPROVAL) 
	{
		...
		//CTLS: set transaction type to offline sale and auth code to "Y1" - start -- mfl
		//vdCTOS_SetTransType(SALE_OFFLINE);		
		//strcpy(srTransRec.szAuthCode, "Y1");
		//CTLS: set transaction type to offline sale and auth code to "Y1" - end -- mfl
...
USHORT usCTOSS_CtlsV3Trans(BYTE *szAmount,BYTE *szOtherAmt,BYTE *szTransType,BYTE *szCatgCode,BYTE *szCurrCode,EMVCL_RC_DATA_EX *stRCDataEx)
{
	...
	vdDebug_LogPrintf("saturn  usCTOSS_CtlsV3Trans szAmount:%s", szAmount);
	vdDebug_LogPrintf("saturn  usCTOSS_CtlsV3Trans szCurrCode:%s", szCurrCode);
	...
	else
	{
		...
		if (d_OK == usResult)
		{
			...
			if ((bOutBuf[4]) == 0xA0)
			{
                ...
                vdDebug_LogPrintf("xxxxusCTOSS_BackToProgress");
			}
...
//	TLVDataParse(stRCDataEx->baAdditionalData, stRCDataEx->usAdditionalDataLen);
	...
	vdDebug_LogPrintf("saturn inCTOSS_V3AnalyzeTransaction");
	...
	vdDebug_LogPrintf("saturn !SCDataEx Track2=[%s][%ld]", stRCDataEx->baTrack2Data, stRCDataEx->bTrack2Len);
	...
	*/
	...
	vdDebug_LogPrintf("saturn stRCDataAnalyze.usTransResult: (%d)", stRCDataAnalyze.usTransResult);
	...
	{
		vdDebug_LogPrintf("saturn Off Approval...  ");
		//srTransRec.byUploaded = CN_FALSE;
		//srTransRec.byOffline = CN_TRUE;
		//vdCTOS_SetTransType(SALE_OFFLINE);
		//strcpy(srTransRec.szAuthCode, "Y1");
...
//		inRet = d_OK;        
	...
	if (inRet == d_OK)
	{
		...
		vdDebug_LogPrintf("saturn usTrack2Len=[%d][%s]...  ",srTransRec.usTrack2Len,srTransRec.szTrack2Data);
		if (srTransRec.usTrack2Len > 0)
		{
			shAnalysisTrack2(srTransRec.szTrack2Data, srTransRec.usTrack2Len);
			vdDebug_LogPrintf("saturn usTrack2Len=[%d][%s]...	",srTransRec.usTrack2Len,srTransRec.szTrack2Data);
...
int inCTOSS_CheckCVMAmount(void)
{
	...
	vdDebug_LogPrintf("AAA - inCTOSS_CheckCVMAmount START");
	...
	if (strlen(strIIT.szCVMLimit)> 0)
	{
		...
		{
			vdDebug_LogPrintf("AAA - inCTOSS_CheckCVMAmount ret d_OK");
		     return d_OK;
		}
    vdDebug_LogPrintf("AAA - inCTOSS_CheckCVMAmount ret d_NO");
	return d_NO;
