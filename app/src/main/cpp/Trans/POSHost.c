...
int inCTOS_CustComputeAndDispTotal(void)
{
	...
	{
		if(get_env_int("CTLS_NO_CONFIRM") == 1)
		    return d_OK;
	}
...
{
    //fix for crash - change szTitle size from 25 to 50
	BYTE szTitle[50+1];
	BYTE szDisMsg[100];
    ...
    { 
        if((srTransRec.byTransType == PREAUTH_COMP || srTransRec.byTransType == PREAUTH_VER) && get_env_int("PRECOMP_VOID") == 1)
        {
			//allow PREAUTH_VER/PREAUTH_COMP void
        }
        else
        {
    ...
    if(srTransRec.byTransType == PREAUTH_COMP || srTransRec.byTransType == PREAUTH_VER)
		srTransRec.byTransType = PREAUTH_COMP_VOID;
    else
...
//USD currency prompts upon voiding & settlement
    ...
    if(srTransRec.fDCCOptin==TRUE)
    	{
				  ...
				  #if 1
				  memset(szDCCAmt, 0x00, sizeof(szDCCAmt)); 
				  strcpy(szDCCAmt, srTransRec.szForeignAmount);
				  #else
                  inDCCAmount=atol((char *)srTransRec.szForeignAmount);
                  sprintf(szDCCAmt,"%ld",inDCCAmount);    
                  #endif
				  
				  vdDebug_LogPrintf("srTransRec.szForeignAmount[%s]", srTransRec.szForeignAmount);
				  vdDebug_LogPrintf("szDCCAmt[%s]", szDCCAmt);
    ...
    if(srTransRec.byTransType == VOID || srTransRec.byTransType == PREAUTH_COMP_VOID)
    {
		memset(szTemp1,0,sizeof(szTemp1));
		vdCTOS_FormatPAN(strIIT.szPANFormat, srTransRec.szPAN, szTemp1, strIIT.fMaskPan);
