...
int inCashGroupIndex;

extern BOOL ErmTrans_Approved;

int inCTOS_CashFlowProcess(void)
{
    int inRet = d_NO;
    ...
    vdCTOS_SetTransType(CASH_ADV);
    
    //display title
    vdDispTransTitle(CASH_ADV);
    ...
    {
        inRet = inCTOS_MultiAPSaveData(d_IPC_CMD_CASH_ADV);
        if(d_OK != inRet)
            return inRet;
    }
    else
