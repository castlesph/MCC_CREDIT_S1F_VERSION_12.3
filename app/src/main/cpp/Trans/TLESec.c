...
int inEncryptTLESec(BYTE *szMsg, int inMsgLen, BYTE *ptrResult)
{
	...
	#ifdef JAVA_TLE
	inGetMACEx(szDataForCompute, inLen+inPaddedLen, szMACKey, szMAC);
	#else
	...
	// ----------------------------------------------------------------------	
	#ifdef JAVA_TLE
	vdTLE_Encrypt(szISOPacket, szDataKey, szEncryptedPacket, inLen);
	#else
...
int inDecryptTLESec(BYTE *szMsg, int inMsgLen, BYTE *ptrResult)
{
	...
	#ifdef JAVA_TLE
	vdTLE_Decrypt(szEncISOData, strTLESEC.szDataKey, szClearPacket, inLen);
	#else
	for(usIndex=0; usIndex<((inLen)/8); usIndex++)
	{
		memset(szDesData, 0, sizeof(szDesData));
...
int inGetMACEx(BYTE *szMsg, int inMsgLen, BYTE *szKey, BYTE *ptrResult)
{
    ...
    memset(szData, 0x00, sizeof(szData));
	memset(szMkey, 0x00, sizeof(szMkey));
	
	inLen=wub_hex_2_str(szMsg, szData, inLen);
	vdDebug_LogPrintf("inGetMACEx szData:%s", szData);

    wub_hex_2_str(szKey, szMkey, 16);
	vdDebug_LogPrintf("inGetMACEx szMkey:%s", szMkey);

	memset(baInBuf, 0x00, sizeof(baInBuf));
	sprintf(baInBuf, "%s|%s", szData, szMkey);
	vdDebug_LogPrintf("inGetMACEx baInBuf:%s", baInBuf);
	
	memset(szMAC, 0x00, sizeof(szMAC));
	//iDES_CBC(d_ENCRYPTION, szMsg, inMsgLen, szMAC, szKey);
	inCallJAVA_TLE_MAC(baInBuf, szMAC, inLen);

    vdDebug_LogPrintf("--inGetMACEx-- szMAC:%s", szMAC);
	memset(szMAC_BCD, 0x00, sizeof(szMAC_BCD));
	wub_str_2_hex(szMAC, szMAC_BCD ,16);
	
	memset(ptrResult, 0x00, sizeof(ptrResult));
	memcpy(ptrResult, szMAC_BCD, 8);

	DebugAddHEX("MAC:", szMAC_BCD, 8);
...
void vdTLE_Encrypt(unsigned char *data, unsigned char *key, unsigned char *result, int inMsgLen)
{
	...
	BYTE baInBuf[2048+1];
	BYTE baOutput[2048+1];
	BYTE baOutputBCD[2048+1];
	int inLen=0, inOutputLen=0;
	
	vdDebug_LogPrintf("--TLE_Encrypt--");
	...
	DebugAddHEX("data:", data, inMsgLen);

    memset(szData, 0x00, sizeof(szData));
	memset(szMkey, 0x00, sizeof(szMkey));
	
	inLen=wub_hex_2_str(data, szData, inLen);
	vdDebug_LogPrintf("TLE_Encrypt szData:%s", szData);

    wub_hex_2_str(key, szMkey, 16);
	vdDebug_LogPrintf("TLE_Encrypt szMkey:%s", szMkey);

	memset(baInBuf, 0x00, sizeof(baInBuf));
	sprintf(baInBuf, "%s|%s", szData, szMkey);
	vdDebug_LogPrintf("TLE_Encrypt baInBuf:%s", baInBuf);
	
	memset(baOutput, 0x00, sizeof(baOutput));
	
	inCallJAVA_TLE_Encrypt(baInBuf, baOutput, inLen);

    inOutputLen=strlen(baOutput);
    vdDebug_LogPrintf("--TLE_Encrypt-- :%s", baOutput);
	memset(baOutputBCD, 0x00, sizeof(baOutputBCD));
	inOutputLen=wub_str_2_hex(baOutput, baOutputBCD ,inOutputLen);
	
	*result=0;
	memcpy(result, baOutputBCD, inOutputLen);

	DebugAddHEX("vdTLE_Encrypt result:", result, inOutputLen);
...
void vdTLE_Decrypt(unsigned char *data, unsigned char *key, unsigned char *result, int inMsgLen)
{
	...
	vdDebug_LogPrintf("--vdTLE_Decrypt--");
	...
	inLen=wub_hex_2_str(data, szData, inLen);
	vdDebug_LogPrintf("vdTLE_Decrypt szData:%s", szData);

    wub_hex_2_str(key, szMkey, 16);
	vdDebug_LogPrintf("vdTLE_Decrypt szMkey:%s", szMkey);

	memset(baInBuf, 0x00, sizeof(baInBuf));
	sprintf(baInBuf, "%s|%s", szData, szMkey);
	vdDebug_LogPrintf("vdTLE_Decrypt baInBuf:%s", baInBuf);
	
	memset(baOutput, 0x00, sizeof(baOutput));
	
	inCallJAVA_TLE_Decrypt(baInBuf, baOutput, inLen);

    inOutputLen=strlen(baOutput);
    vdDebug_LogPrintf("--vdTLE_Decrypt-- :%s", baOutput);
	memset(baOutputBCD, 0x00, sizeof(baOutputBCD));
	inOutputLen=wub_str_2_hex(baOutput, baOutputBCD ,inOutputLen);
	...
	*result=0;
	...
	DebugAddHEX("vdTLE_Decrypt result:", result, inOutputLen);
