...
//fix for crash - change size of szGlobalAPName from 25 to 50
...
unsigned char gblszMarginRatePercentage[10+1], gblszExchangeRate[12+1], gblszDCCCurrencySymbol[3+1],  gblszCurrencyCode[3+1], gblszForeignAmount[20+1], gblszLocalAmount[20+1], gblszCurrForeignAmount[20+1], 


//fix for crash
	//gblszDCCCurrBaseAmount[15+1], 
	gblszDCCCurrBaseAmount[20+1], 
	gblszDCCTipAmount[12+1], 
	//gblszCurrDCCTipAmount[15+1];
	gblszCurrDCCTipAmount[20+1];
int gblinMinorUnit;
...
void vdCTOS_SyncHostDateTime()
{
    ...
    char szSysYearYyyy[4+1] = {0};
    char szSysYear[2+1]= {0};
    BYTE    bSysYear = 0;
	vdDebug_LogPrintf("--vdCTOS_SyncHostDateTime--");
  ...
  //vdDebug_LogPrintf("year[%02X],time[%02x:%02x:%02x]date[%02x][%02x]",SetRTC.bYear,srTransRec.szTime[0],srTransRec.szTime[1],srTransRec.szTime[2],srTransRec.szDate[0],srTransRec.szDate[1]);
	...
	strcat(szDateTime,"x|");
    inCTOSS_GetEnvDB("SYSYEAR", szSysYearYyyy);
    vdDebug_LogPrintf("Pre-Saved szSysYearYyyy %s", szSysYearYyyy);
    ...
    strcat(szDateTime,"|");
    strncat(szDateTime, szSysYearYyyy,4);
    //CTOS_RTCSet(&SetRTC);
...
int inCTOS_CheckMustSettle(void)
{
    ...
    if(1 == inEnable)
    {
        //remove - bot beinng used - S1F fix
        #if 0
...
short inCTOS_LoadCDTIndex(void)
{
    ...
    #if 1
	inCDTReadMultiNew(srTransRec.szPAN, &inFindRecordNum);
	#else
...
int inCTOS_GetCardFields(void)
{
 ...
 SWIPE_AGAIN:
	...
	vdDebug_LogPrintf("saturn check ing_KeyPressed");
        ...
        if(strlen(szPAN) <= 0)
        {
            vdDebug_LogPrintf("saturn before CTOS_KBDInKey");
        	...
        	if (byKeyBuf)
        	{
            	...
            	if(fInsertChipOnly == VS_TRUE && bKey != d_KBD_CANCEL)
                	continue;
            	...
            	else if(bKey == d_KBD_CANCEL)
            	{
					...
					vdDebug_LogPrintf("saturn  CTOS_KBDInKey continue");
                continue;
            	}
...
#define ITEMS_PER_PAGE          4
    ...
    {
		vdDebug_LogPrintf("1.inCTOS_CheckAndSelectMutipleMID");
        if(inGetMerchantPassword() != d_OK)
        	return d_NO;
        vdDebug_LogPrintf("2.inCTOS_CheckAndSelectMutipleMID");
		if(inGetMKEPassword() != d_OK)
			return d_NO;
    }
	else
		vdDebug_LogPrintf("3.inCTOS_CheckAndSelectMutipleMID");
...
int inCTOS_GetTxnPassword(void)
{
	 ...
	 strcpy(szMsg, "4");
	 strcat(szMsg, "|");
	 if(srTransRec.byTransType == CLEAR_BATCH)
	     strcat(szMsg, "8");
	 else
	 	strcat(szMsg, "6");
	 strcat(szMsg, "|");
	 ...
	 //strPIT.inPasswordLevel=GENERATE_PW;
     ...
     switch(srTransRec.byTransType)
     {
		  ...
		  else if(strPIT.inPasswordLevel == GENERATE_PW)
          {
               shMinLen = PWD_MIN_LEN;
               shMaxLen = PWD_MAX_LEN;
			   ...
			   //Bret = InputStringUI(0x02, 0x02, szOutput, &shMaxLen, shMinLen, d_INPUT_TIMEOUT, szPassword, "ENTER PASSWORD");
               memset(generated_data, 0, sizeof(generated_data));
			   generate_password("10000001", generated_data);
			   strcpy(szPassword,szMsg);
			   strcat(szPassword, "|");
			   strcat(szPassword, generated_data);
			   memset(szOutput,0,sizeof(szOutput));
			   Bret = InputStringUI(0x02, 0x02, szOutput, &shMaxLen, shMinLen, d_INPUT_TIMEOUT, szPassword);
			   vdDebug_LogPrintf("3. ENGINEERPW szOutput[%s], Bret[%d]", szOutput, Bret);
               ...
               if(d_KBD_CANCEL == Bret)
                    ...
                    return Bret;			  
               else if(strcmp(szOutput,generated_data) == 0)
               {
                    //clearLine(7);
                    //clearLine(8);
                    return d_OK;
...
int inCTOS_SelectHost(void)
{
	...
	vdDebug_LogPrintf("1.saturn --inCTOS_SelectHost--");
   ...
   inGlobalCount = 0; // fix for 1. Performed sale transaction 2. Prints DE 63 first: Parsing not in order
	...
	inDatabase_TerminalCloseDatabase();
	vdDebug_LogPrintf("2.saturn --inCTOS_SelectHost--");
    return (d_OK);
}
#if 1
...
BYTE szDisMsg[100];
    ...
    if(usResult != PP_OK)
    {
        if(strcmp((char *)srTransRec.szRespCode, "00") ==  0)
        	{
				vdDebug_LogPrintf("1.EMV Decline");
            //vdSetErrorMessage("EMV Decline");
                memset(szTitle, 0x00, sizeof(szTitle));
                szGetTransTitle(srTransRec.byTransType, szTitle);
                strcpy(szDisMsg, szTitle);
                ...
                usCTOSS_LCDDisplay(szDisMsg);
                CTOS_Beep();
				memset(srTransRec.szRespCode, 0, sizeof(srTransRec.szRespCode));
				memcpy(srTransRec.szRespCode, "XX", 2);
        	}
        return EMV_CRITICAL_ERROR;
    }
    ...
    switch( EMVtagVal[0] & 0xC0)
    {
        case 0: //Declined --- AAC
            ...
            {
				vdDebug_LogPrintf("2.EMV Decline");
                //vdSetErrorMessage("EMV Decline");
                memset(szTitle, 0x00, sizeof(szTitle));
                szGetTransTitle(srTransRec.byTransType, szTitle);
                strcpy(szDisMsg, szTitle);
        ...
        default:
			vdDebug_LogPrintf("3.EMV Decline");
            //vdSetErrorMessage("EMV Decline");
            memset(szTitle, 0x00, sizeof(szTitle));
            szGetTransTitle(srTransRec.byTransType, szTitle);
            strcpy(szDisMsg, szTitle);
                  ...
                  if (inRet == d_KBD_CANCEL )
                   ...
                   else if(inRet>=1)
                   {
                        ...
                        else
                        {
                             ...
                             //vdClearBelowLine(8);
                             //remove  not being used anymore - S1F fix
                             #if 0
                             memset(szDisMsg, 0x00, sizeof(szDisMsg));
                             strcpy(szDisMsg, szTitle);
                             strcat(szDisMsg, "|");
...
int inCTOS_GetCardFieldsCtls(void)
{
    ...
    BYTE szManualEntryFlag[2];
        ...
        EMVCL_RC_DATA_EX stRCDataEx;
		//fix for crash - change size of szCatgCode and szCurrCode from 3 to 10
        BYTE szOtherAmt[12+1],szTransType[2+1],szCatgCode[10+1],szCurrCode[10+1];
        BOOL fMaxCTLSAmt = FALSE;
        long amt1 = 0;
        long amt2 = 0;
    #endif
 ...
 SWIPE_AGAIN:
    ...
    //usCARDENTRY(szDisMsg);
	vdDebug_LogPrintf("saturn inside while loop");
	...
	vdDebug_LogPrintf("saturn check ing_KeyPressed");
        ...
        //{
#if 0   
			vdDebug_LogPrintf("saturn check CTOS_KBDInKey");
            CTOS_KBDInKey(&byKeyBuf);
        	//if ((byKeyBuf) || (d_OK == inCTOS_ValidFirstIdleKey()))
			if (byKeyBuf)
        	{
                ...
                vdDebug_LogPrintf("saturn keypressed");
            	memset(srTransRec.szPAN, 0x00, sizeof(srTransRec.szPAN));
            	if(d_OK == inCTOS_ValidFirstIdleKey())
            	{
                	bKey = srTransRec.szPAN[0] = chGetFirstIdleKey();
        ...
        if(bySC_status & d_MK_SC_PRESENT)
        {
			vdDebug_LogPrintf("2.inCTOS_GetCardFieldsCtls");
			...
			vdDebug_LogPrintf("1.check processing display time");
            vdDisplayMessageStatusBox(1, 8, "PROCESSING", MSG_PLS_WAIT, MSG_TYPE_PROCESS);
			vdDebug_LogPrintf("2.check processing display time");
			vdDebug_LogPrintf("saturn card entry ICC");
            ...
            vdDebug_LogPrintf("3.inCTOS_GetCardFieldsCtls");
            break;
        }
		...
		else
		{
            ...
            if(ulAPRtn == d_OK)
            {
				//vdDisplayMessageBox(1, 8, "", "PLEASE REMOVE CARD", "", MSG_TYPE_ERROR);
                //TEST
                CTOS_Beep();
                vdCTOS_SetTransEntryMode(CARD_ENTRY_WAVE);
				vdDisplayMessageStatusBox(1, 8, "PROCESSING", MSG_PLS_WAIT, MSG_TYPE_PROCESS);
...
int inCTOS_SelectTelco(void)
{
	...
	if (GPTid > 0)
	{
		...
		if (memcmp(szGPTName,"PUBLIC IP",9) == 0 
			|| memcmp(szGPTName,"PUBLIC URL",10) == 0)
		{
			inSSLRead(1);
			//enable POST mode, rounting to cardbiz host
			strSSL.inAmexETHDirect = 1;
			inSSL_Update(1);
			...
			if (strTCT.byERMMode != 0)
			{
				memset(szGPTPriTxnHostIP,0x00,sizeof(szGPTPriTxnHostIP));
				//MCC UAT host
				//strcpy(szGPTPriTxnHostIP,"211.24.105.203");
				//inGPTPriTxnHostPortNum = 10003;

				//MCC Production ERM host and port
				strcpy(szGPTPriTxnHostIP,"103.240.178.166");
				inGPTPriTxnHostPortNum = 8246;
				inERMTCT_IPUpdate(szGPTPriTxnHostIP, szGPTPriTxnHostIP, inGPTPriTxnHostPortNum, inGPTPriTxnHostPortNum, 1);
			}
		}
		else
		{
			inSSLRead(1);
			//disable POST mode, Directly to MCC
			strSSL.inAmexETHDirect = 0;
			inSSL_Update(0);
...
void vdGetCRC(void)
{
	...
	if ((file = fopen ("V5S_MCCNH", "rb")) != NULL)
	{
		...
		if (fHD)
		{
			...
			put_env_char("V5S_MCCNH_CRC", "143ADD579");
		}
		else
		{
...
{
     BYTE key;
     unsigned char szIncludesExchangeRate[100], szExchangeRate[100], szDCCCurrencySymAmt[24+1], szLocalCurrencySymAmt[45], szLocalCurrencySymAmtX[45], szTotalAmt[12+1], szTotalAmountformat[45], szTotalDCCAmountformat[45], szDCCCurrencySymAmtOK[18+1], szDCCAmt[12+1];
     int inMinorUnit;
     ...
     BYTE szDccAmtTotal[20+1], szDccAmtTotalFormatted[20+1], szDccAmtTotalBuff[20+1], szDCCForeignAmountLessTip3MinorUnits[12+1];
   ...
   //inCSTReadCurrencySymbol(srTransPara->szCurrencyCode, srTransPara->szDCCCurrencySymbol, srTransPara->szAmountFormat);
   //fix for crash - only include 2 decimal places, for formatting
   sprintf(szDccAmtTotal,"%lf",ldDccAmtTotal);
   //memset(szDccAmtTotal, 0x00, sizeof(szDccAmtTotal));
   //strcpy(szDccAmtTotal, szDccAmtTotalBuff);
   vdDebug_LogPrintf("saturn1 test 1 szDccAmtTotal %s", szDccAmtTotal);

   //TESTALNG
 #if 1
   memset(szTempBuffer, 0x00, sizeof(szTempBuffer));
   for (inCounter = 0; ;inCounter++){
   		if (szDccAmtTotal[inCounter] == '.')
			break;
		else
			szTempBuffer[inCounter] = szDccAmtTotal[inCounter];
		vdDebug_LogPrintf("szTempBuffer = %s", szTempBuffer);
   }
   
   
   vdDebug_LogPrintf("saturn1 test 3 szTempBuffer %s", szTempBuffer);
   memset(szDccAmtTotal, 0x00, sizeof(szDccAmtTotal));
   strcpy(szDccAmtTotal, szTempBuffer);
#endif
   //TESTLANG

   vdDebug_LogPrintf("saturn1 test 2 szDccAmtTotal %s", szDccAmtTotal);
   ...
   vdDebug_LogPrintf("amount format %s", srTransPara->szAmountFormat);
	...
	vdDebug_LogPrintf("test lang  szAmtBuff%s", szAmtBuff);
    if(srTransPara->inMinorUnit==0)
		purgeleadingchar('0',szAmtBuff,szAmtBuff2);

	vdDebug_LogPrintf("x.szAmtBuff2[%s]", szAmtBuff2);
    ...
    memset(szExchangeRate, 0, sizeof(szExchangeRate));
	 ...
	 vdDebug_LogPrintf("dcc crashtest 1");
     if(srTransPara->inMinorUnit==0)
	 ...
	 vdDebug_LogPrintf("dcc crashtest 2");

	 //add initialize for all variables
	 memset(gblszDCCCurrBaseAmount, 0x00, sizeof(gblszDCCCurrBaseAmount));
	 memset(gblszCurrDCCTipAmount, 0x00, sizeof(gblszCurrDCCTipAmount));
	 memset(gblszCurrDCCTipAmount, 0x00, sizeof(gblszCurrDCCTipAmount));
	 memset(gblszForeignAmount, 0x00, sizeof(gblszForeignAmount));
	 memset(gblszMarginRatePercentage, 0x00, sizeof(gblszMarginRatePercentage));
	 memset(gblszExchangeRate, 0x00, sizeof(gblszExchangeRate));
	 memset(gblszDCCCurrencySymbol, 0x00, sizeof(gblszDCCCurrencySymbol));
	 memset(gblszLocalAmount, 0x00, sizeof(gblszLocalAmount));
	 memset(gblszCurrForeignAmount, 0x00, sizeof(gblszCurrForeignAmount));


     sprintf(szLocalCurrencySymAmt, "%s %s",strCST.szCurSymbol,szTotalAmountformat); vdDebug_LogPrintf("dcc crashtest 2.1");
     sprintf(gblszDCCCurrBaseAmount, "%s %s", strCST.szCurSymbol, szDccAmtTotalFormatted); vdDebug_LogPrintf("dcc crashtest 2.2");
     sprintf(gblszCurrDCCTipAmount, "%s %s",srTransPara->szDCCCurrencySymbol, szDCCTip); vdDebug_LogPrintf("dcc crashtest 2.3");
     sprintf(szDCCCurrencySymAmtOK,"%s", szDCCCurrencySymAmt); vdDebug_LogPrintf("dcc crashtest 2.4");
     sprintf(szLocalCurrencySymAmtX, "%s",szLocalCurrencySymAmt); vdDebug_LogPrintf("dcc crashtest 2.5");
		 vdDebug_LogPrintf("dcc crashtest 3");

    //fix for crash
    //memcpy(&gblszDCCCurrBaseAmount,szDccAmtTotalFormatted,sizeof(gblszDCCCurrBaseAmount));
    memcpy(&gblszDCCCurrBaseAmount,szDccAmtTotalFormatted,strlen(szDccAmtTotalFormatted));
	vdDebug_LogPrintf("dcc crashtest 3.1 %s", szDccAmtTotalFormatted);

    //memcpy(&gblszForeignAmount,(char *)srTransPara->szForeignAmount,sizeof(gblszForeignAmount));
	memcpy(&gblszForeignAmount,(char *)srTransPara->szForeignAmount,strlen((char *)srTransPara->szForeignAmount));
	vdDebug_LogPrintf("dcc crashtest 3.2 %s", srTransPara->szForeignAmount);

    //memcpy(&gblszMarginRatePercentage,(char *)srTransPara->szMarginRatePercentage,sizeof(gblszMarginRatePercentage));
    memcpy(&gblszMarginRatePercentage,(char *)srTransPara->szMarginRatePercentage,strlen((char *)srTransPara->szMarginRatePercentage));
	vdDebug_LogPrintf("dcc crashtest 3.3 %s", srTransPara->szMarginRatePercentage);

    //memcpy(&gblszExchangeRate,(char *)srTransPara->szExchangeRate,sizeof(gblszExchangeRate));
    memcpy(&gblszExchangeRate,(char *)srTransPara->szExchangeRate,strlen((char *)srTransPara->szExchangeRate));
	vdDebug_LogPrintf("dcc crashtest 4");

    //memcpy(&gblszDCCCurrencySymbol, (char *)srTransPara->szDCCCurrencySymbol,sizeof(gblszDCCCurrencySymbol));
    memcpy(&gblszDCCCurrencySymbol, (char *)srTransPara->szDCCCurrencySymbol,strlen((char *)srTransPara->szDCCCurrencySymbol));
	vdDebug_LogPrintf("dcc crashtest 5 %s", gblszDCCCurrencySymbol);
    //memcpy(&gblszLocalAmount,(char *)szLocalCurrencySymAmt,sizeof(gblszLocalAmount));
    memcpy(&gblszLocalAmount,(char *)szLocalCurrencySymAmt,strlen((char *)szLocalCurrencySymAmt));
	vdDebug_LogPrintf("dcc crashtest 6 %s", gblszLocalAmount);
    //memcpy(&gblszCurrForeignAmount,(char *)szDCCCurrencySymAmt,sizeof(gblszCurrForeignAmount));

	vdDebug_LogPrintf("before crash 7 %s",szDCCCurrencySymAmt);
    memcpy(&gblszCurrForeignAmount,(char *)szDCCCurrencySymAmt,strlen((char *)szDCCCurrencySymAmt));
	vdDebug_LogPrintf("dcc crashtest 7 %s", gblszCurrForeignAmount);
	//end of fixes
...
int vdCTOS_EditHost(void)
{
	...
	// Host Name
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szHostLabel", "Host Name", strHDT.szHostLabel, 16, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	
	// Host Enable
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fHostEnable", "Host Enable?", strHDT.fHostEnable, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// TLE Enable
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fHostTLESec", "TLE Enable?", strHDT.fHostTLESec, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// TID
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szTID", "Terminal ID", strMMT[0].szTID, 8, TRUE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	     ...
	     inLenMID = 15;
              ...
              memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
              sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szMID", "Merchant ID", strMMT[0].szMID, inLenMID, TRUE);
              strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
              strcat(strWriteFile.szWriteData, "\n");
	...
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szTPDU", "TPDU", szTempBuf, 10, TRUE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	...
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szNII", "NII", szTempBuf, 4, TRUE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// System PWD
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szSystemPW", "System PWD", strTCT.szSystemPW, PWD_LEN, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	
	// Engineer PWD
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szEngineerPW", "Engineer PWD", strTCT.szEngineerPW, PWD_LEN, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// Tip Allow
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fTipAllowFlag", "Tip Allow?", strHDT.fTipAllowFlag, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	...
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szBatchNo", "Batch No", szTempBuf, BATCH_NO_ASC_SIZE, TRUE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	...
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szTraceNo", "Trace No", szTempBuf, BATCH_NO_ASC_SIZE, TRUE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	...
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szInvoiceNo", "Invoice No", szTempBuf, BATCH_NO_ASC_SIZE, TRUE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "ulTipPercent", "Tip Percent", strHDT.ulTipPercent, 3, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	...
	switch (inKey)
	{
		...
		case d_OK:
			...
			memset(szTemp, 0x00, sizeof(szTemp));
			GetDelimitedString(szOutputBuf, 16, '|', szTemp); // fHostEnable
			strHDT.ulTipPercent = atoi(szTemp);
...
int vdCTOS_EditIP(void)		//tine:  modify to return int value, for handling Cancel function
{
	...
	// inCommunicationMode
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inCommunicationMode", "Connection Type", strCPT.inCommunicationMode, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// inIPHeader
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inIPHeader", "IP Header", strCPT.inIPHeader, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szPriTxnHostIP
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szPriTxnHostIP", "Primary Host IP", strCPT.szPriTxnHostIP, 45, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// inPriTxnHostPortNum
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inPriTxnHostPortNum", "Primary Host Port", strCPT.inPriTxnHostPortNum, 6, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szSecTxnHostIP
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szSecTxnHostIP", "Secondary Host IP", strCPT.szSecTxnHostIP, 45, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// inSecTxnHostPortNum
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inSecTxnHostPortNum", "Secondary Host Port", strCPT.inSecTxnHostPortNum, 6, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szPriSettlementHostIP
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szPriSettlementHostIP", "Settle Primary Host IP", strCPT.szPriSettlementHostIP, 45, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// inPriSettlementHostPort
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inPriSettlementHostPort", "Settle Primary Host Port", strCPT.inPriSettlementHostPort, 6, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szSecSettlementHostIP
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szSecSettlementHostIP", "Settle Secondary Host IP", strCPT.szSecSettlementHostIP, 45, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// inSecSettlementHostPort
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inSecSettlementHostPort", "Settle Secondary Host Port", strCPT.inSecSettlementHostPort, 6, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fDHCPEnable
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fDHCPEnable", "[0-Static/1-DHCP]", strTCP.fDHCPEnable, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");		

	// szTerminalIP
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szTerminalIP", "Terminal IP", strTCP.szTerminalIP, 30, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szSubNetMask
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szSubNetMask", "SubNet Mask IP", strTCP.szSubNetMask, 30, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szGetWay
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szGetWay", "Gateway IP", strTCP.szGetWay, 30, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szHostDNS1
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szHostDNS1", "DNS1 IP", strTCP.szHostDNS1, 30, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szHostDNS2
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szHostDNS2", "DNS2 IP", strTCP.szHostDNS2, 30, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
...
void vdCTOS_EditTerminal(void)
{
	...
	// inMenuid
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inMenuid", "Menu ID", strTCT.inMenuid, 3, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// inMenuidNHProfileMenu
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inMenuidNHProfileMenu", "Profile Menu ID", strTCT.inMenuidNHProfileMenu, 3, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	...
	// fManualEntry
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fManualEntry", "Enable Manual Entry?", strTCT.fManualEntry, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fRegUSD
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fRegUSD", "Terminal USD Reg?", strTCT.fRegUSD, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fTSNEnable
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fTSNEnable", "Enable TSN (Terminal SN)?", strTCT.fTSNEnable, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szOfflineCeilingAmt
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szOfflineCeilingAmt", "Offline Ceilling Amt", strTCT.szOfflineCeilingAmt, 12, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fPrintReceipt
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fPrintReceipt", "Enable Print Receipt?", strTCT.fPrintReceipt, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fEMVOnlinePIN
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fEMVOnlinePIN", "Enable EMV Offline PIN?", strTCT.fEMVOnlinePIN, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// byIdleSaleType
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "byIdleSaleType", "Idle Sale Type", strTCT.byIdleSaleType, 2, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fIdleSwipeAllow
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fIdleSwipeAllow", "Enable Idle Swipe?", strTCT.fIdleSwipeAllow, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// fIdleInsertAllow
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fIdleInsertAllow", "Enable Idle Insert?", strTCT.fIdleInsertAllow, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// ISO Log On/Off
    memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
    sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inISOLogger", "ISO LOG 1-ON 0-OFF", strTCT.inISOLogger, 1, FALSE);
    strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
    strcat(strWriteFile.szWriteData, "\n");
        
    // inISOLog No. of Trans Limit
    memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
    sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "inISOTxnLogLimit", "ISO LOG No. of Trans Limit", strTCT.inISOTxnLogLimit, 3, FALSE);
    strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
    strcat(strWriteFile.szWriteData, "\n");
...
int vdCTOS_EditPrinter(void)		////tine:  modify to return int value, for handling Cancel function
{
	...
	// szRctHdr1
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctHdr1", "Receipt Header Line 1", strMMT[0].szRctHdr1, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szRctHdr2
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctHdr2", "Receipt Header Line 2", strMMT[0].szRctHdr2, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szRctHdr3
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctHdr3", "Receipt Header Line 3", strMMT[0].szRctHdr3, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szRctHdr4
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctHdr4", "Receipt Header Line 4", strMMT[0].szRctHdr4, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	
	// szRctHdr5
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctHdr5", "Receipt Header Line 5", strMMT[0].szRctHdr5, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szRctFoot1
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctFoot1", "Receipt Footer Line 1", strMMT[0].szRctFoot1, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szRctFoot2
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctFoot2", "Receipt Footer Line 2", strMMT[0].szRctFoot2, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// szRctFoot3
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%s|%d|%d", FLD_TYPE_NUMERIC, "szRctFoot3", "Receipt Footer Line 3", strMMT[0].szRctFoot3, 40, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
...
void vdCTOS_ECRSettings(void)
{
	...
	// ECR Flag
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fECR", "ECR Flag", strTCT.fECR, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// ECR Port
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "byRS232ECRPort", "ECR Port", strTCT.byRS232ECRPort, 2, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// ECR Baud Rate
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "byRS232ECRBaud", "ECR Baud Rate", strTCT.byRS232ECRBaud, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
	
	// ECR Credit
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fECRSale", "ECR Credit?", strTCT.fECRSale, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// ECR Debit
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fECRBancnet", "ECR Bancnet?", strTCT.fECRBancnet, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");

	// ECR QRPay
	memset(strWriteFile.szTemp, 0x00, sizeof(strWriteFile.szTemp));
	sprintf(strWriteFile.szTemp, "%d|%s|%s|%d|%d|%d", FLD_TYPE_NUMERIC, "fECRQRPay", "ECR QRPay?", strTCT.fECRQRPay, 1, FALSE);
	strcat(strWriteFile.szWriteData, strWriteFile.szTemp);
	strcat(strWriteFile.szWriteData, "\n");
...
int inGetMKEPassword(void)
{
    ...
    if(get_env_int("MKE_PWD") == 1 && srTransRec.byEntryMode == CARD_ENTRY_MANUAL)
    {
	//#2 added to fix - Issue is MKE PASSWORD is having wrong password when first transaction done is 
	//OFFLINE, PREAUTH, PREAUTH VOID, INSTALLMENT, and PREAUTH COMPLETION even if inputted pw is correct. You need to input correct pw first in SALE txn before all pws in other functions will work.
	inTCTRead(1);

		vdDebug_LogPrintf("inGetMKEPassword strTCT.szMKEPassword [%s]", strTCT.szMKEPassword);
        ...
        while(1)
        {
            ...
            strcat(szMsg, szTitle);
            strcat(szMsg, "|");
            strcat(szMsg, "MKE PASSWORD");
            strcat(szMsg, "|");
            strcat(szMsg, strTCT.szMKEPassword);
...
void vdReadUpdateMMT(void) 
{
	if(get_env_int("UPDATE_MMT") != 1)
		return;
		
	vdDebug_LogPrintf("start.vdReadUpdateMMT");
	 
	inDatabase_TerminalOpenDatabase();
	
	memset(&strMMT,0x00,sizeof(STRUCT_MMT));	
    inMMTReadRecordEx(1, 1);
    if(strMMT[0].fMMTEnable == 1)
		inMMTSaveEx2(1);

	memset(&strMMT,0x00,sizeof(STRUCT_MMT));
    inMMTReadRecordEx(1, 2);
    if(strMMT[0].fMMTEnable == 1)
        inMMTSaveEx2(2);

    memset(&strMMT,0x00,sizeof(STRUCT_MMT));
    inMMTReadRecordEx(1, 3);
    if(strMMT[0].fMMTEnable == 1)
        inMMTSaveEx2(3);

    memset(&strMMT,0x00,sizeof(STRUCT_MMT));
    inMMTReadRecordEx(1, 4);
    if(strMMT[0].fMMTEnable == 1)
		inMMTSaveEx2(4);

    memset(&strMMT,0x00,sizeof(STRUCT_MMT));
    inMMTReadRecordEx(1, 5);
    if(strMMT[0].fMMTEnable == 1)
        inMMTSaveEx2(5);

	inDatabase_TerminalCloseDatabase();

	vdDebug_LogPrintf("last.vdReadUpdateMMT");
}
...
char *addBCD(char *szAddends1, char *szAddends2, char *szSum);
void subtractBCD(char *szMinuend, char *szSubtrahend, char *szDifference);
void addBCD2(char *szAddends1, char *szAddends2, char *szSum);
void bcd_str(char *bcd, int len, char *str);
...
int main() {
    ...
    char test[12+1];
    char carry;
    char chValue1, chSum1, chCarry1;
    char chValue2, chSum2, chCarry2;
    //char sum1;
    
    
    //memcpy(a, "\x02\x01\x90\x00\x01\x69", 6);
    //memcpy(b, "\x00\x01\x90\x00\x03\x49", 6);
    
    
    
    memcpy(a, "\x00\x00\x00\x30\x11\x11", 6);
    memcpy(b, "\x00\x00\x00\x00\x96\x99", 6); //977
    addBCD2(a, b, c);
    //memcpy(b, "\x00\x00\x00\x00\x06\x00", 6);
    
    //(02)(03)(ffffff80)(00)(05)(18)
...
char *addBCD(char *szAddends1, char *szAddends2, char *szSum)
{
    char a[6+1];
    char b[6+1];
    char chValue1=0x00, chSum1=0x00, chCarry1=0x00;
    char chValue2=0x00, chSum2=0x00, chCarry2=0x00;
    int inIndex=5;
    char c=0x00,d=0x00;
    char szSum1[6+1];
    ...
    memcpy(a, szAddends1, 6);
    memcpy(b, szAddends2, 6);
    
    for(inIndex=5; inIndex>=0; inIndex--)
    {
        //printf("a:%02x\n", a[inIndex]);
        //printf("b:%02x\n", b[inIndex]);
    
        chSum1 = (a[inIndex] & 0x0f) + (b[inIndex] & 0x0f) + (chCarry2 >> 4);
        //printf("chSum1:%02x\n", chSum1);
        chValue1 = chSum1 % 10;
        //printf("chValue1:%02x\n", chValue1);
        chCarry1 =chSum1 / 10;
       ...
       // printf("chCarry2:%02x\n\n", chCarry2);
        ...
        //printf("%d-total:%02x\n",inIndex, chValue1 + (chValue2 << 4));
        
        szSum1[inIndex] = (char)(chValue1 + (chValue2 << 4));
    }
    memcpy(szSum, szSum1, 6);
}
#endif

void addBCD(char *szAddends1, char *szAddends2, char *szSum)
{
    int inIndex=0, inIndex2=0, inMinuendIdx=11, inSubtrahendIdx=11;
    int inStart=0, inCarry=0, inValue=0;
    int inAddends1[12];
    int inAddends2[12];
    int inSum[12];
    
    char szTemp[2+1], szTemp2[12+1], szTemp3[12+1], szTempSum[12+1];
    
    memset(szTemp2, 0, sizeof(szTemp2));
    bcd_str(szAddends1, 6, szTemp2);
    printf("szTemp2:%s\n", szTemp2);

    memset(szTemp3, 0, sizeof(szTemp3));
    bcd_str(szAddends2, 6, szTemp3);
    printf("szTemp3:%s\n", szTemp3);
    
    for(inIndex=11; inIndex>=0; inIndex--)
    {
        memset(szTemp, 0x00, sizeof(szTemp));
        memcpy(szTemp, &szTemp2[inIndex], 1);
        inAddends1[inIndex] = atoi(szTemp);
        //priinSum[inIndex]x]%d\n", inMinuend[inIndex]);

        memset(szTemp, 0x00, sizeof(szTemp));
        memcpy(szTemp, &szTemp3[inIndex], 1);
        inAddends2[inIndex] = atoi(szTemp);
        //printf("inSubtrahend[inIndex]%d\n", inSubtrahend[inIndex]);
    }
    
    for(inIndex=11; inIndex>=0; inIndex--)
    {
        inValue=inAddends1[inIndex]+inAddends2[inIndex]+inCarry;
        inSum[inIndex]=inValue%10;
        inCarry=inValue/10;
        printf("inSum[inIndex]:%d\n", inSum[inIndex]);
    }

	memset(szTempSum, 0, sizeof(szTempSum));
    sprintf(szTempSum,"%d%d%d%d%d%d%d%d%d%d%d%d", inSum[0],  inSum[1],inSum[2], inSum[3], inSum[4], inSum[5], inSum[6], inSum[7], inSum[8], inSum[9], inSum[10], inSum[11]);
	wub_str_2_hex(szTempSum, szSum, 12);
...
void subtractBCD(char *szMinuend, char *szSubtrahend, char *szDifference)
{
    ...
    char szTemp[2+1], szTemp2[12+1], szTemp3[12+1], szTempDifference[12+1];
    
    memset(szTemp2, 0, sizeof(szTemp2));
    bcd_str(szMinuend, 6, szTemp2);
    printf("szTemp2:%s\n", szTemp2);

    memset(szTemp3, 0, sizeof(szTemp3));
    bcd_str(szSubtrahend, 6, szTemp3);
    printf("szTemp3:%s\n", szTemp3);
    ...
    for(inIndex=11; inIndex>=0; inIndex--)
    {
        memset(szTemp, 0x00, sizeof(szTemp));
        memcpy(szTemp, &szTemp2[inIndex], 1);
        inMinuend[inIndex] = atoi(szTemp);
        //printf("inMinuend[inIndex]%d\n", inMinuend[inIndex]);

        memset(szTemp, 0x00, sizeof(szTemp));
        memcpy(szTemp, &szTemp3[inIndex], 1);
        inSubtrahend[inIndex] = atoi(szTemp);
        //printf("inSubtrahend[inIndex]%d\n", inSubtrahend[inIndex]);
    }
    
    for(inIndex=11; inIndex>=0; inIndex--)
    {
        if(inMinuend[inIndex] < inSubtrahend[inIndex])
        {
            inStart=inIndex-1;
            for(inIndex2=inStart; inIndex2>=0; inIndex2--)
            {
                if(inMinuend[inIndex2] > 0)
                {
                    //printf("1.inMinuend[%d]%d\n", inIndex2, inMinuend[inIndex2]);
                    inMinuend[inIndex2]-=1;
                    //printf("2.inMinuend[inIndex2]%d\n", inMinuend[inIndex2]);
                    inMinuend[inIndex]+=10;
                    inDifference[inIndex] = inMinuend[inIndex] - inSubtrahend[inIndex];
        ...
        printf("inDifference[inIndex]:%d\n", inDifference[inIndex]);
    }

    memset(szTempDifference, 0, sizeof(szTempDifference));
	sprintf(szTempDifference,"%d%d%d%d%d%d%d%d%d%d%d%d", inDifference[0],  inDifference[1], inDifference[2], inDifference[3], inDifference[4], inDifference[5], inDifference[6], inDifference[7], inDifference[8], inDifference[9], inDifference[10], inDifference[11]);
    wub_str_2_hex(szTempDifference, szDifference, 12);
		
	printf("szTempDifference:%s\n", szTempDifference);
}
...
void generate_password(char *szIndata, char *szOutData)
{
    //time_t t = time(NULL);
    //struct tm tm = *localtime(&t);
    CTOS_RTC SetRTC;
    char szDate[8+1];
    int inDate=0, inData=0, inLastDigit=0, inValue=0;
    char chTemp1[2+1], chTemp2[2+1];
    ...
    vdDebug_LogPrintf("generate_password: szDate:%s\n", szDate);
    
    memset(chTemp1, 0, sizeof(chTemp1));
    chTemp1[0]=szIndata[7];
    inData=atoi(chTemp1);
 
    memset(chTemp2, 0, sizeof(chTemp1));
    chTemp2[0]=szDate[7];
    inDate=atoi(chTemp2);
    
    inLastDigit=inData+inDate;
    vdDebug_LogPrintf("generate_password: inLastDigit:%d\n", inLastDigit);
    
    for(int inIndex=0; inIndex < 8; inIndex++)
    {
        memset(chTemp1, 0, sizeof(chTemp1));
        chTemp1[0]=szIndata[inIndex];
        inData=atoi(chTemp1);
        //printf("inData:%d\n", inData);
        
        memset(chTemp2, 0, sizeof(chTemp2));
        chTemp2[0]=szDate[inIndex];
        inDate=atoi(chTemp2);
        //printf("inDate:%d\n", inDate);
        
        inValue=(inData+inDate+inLastDigit) * inLastDigit;
        if(inValue >= 10)
            inValue%=10;

        printf("value:%d\n", inValue);
        sprintf(&szOutData[inIndex], "%d", inValue);
        
    }
}

void checkSystemDate(void)
{
	CTOS_RTC SetRTC;
	BYTE szSysYear[8+1]={0};
	BYTE szCurrentYear[8+1]={0};
	...
	vdDebug_LogPrintf("--checkSystemDate--");
	CTOS_RTCGet(&SetRTC);
	vdDebug_LogPrintf("sys year[%02d],month[%02d],year[%02d]time[%02d][%02d][%02d]",SetRTC.bYear,SetRTC.bMonth,SetRTC.bDay,SetRTC.bHour,SetRTC.bMinute,SetRTC.bSecond);
	memset(szCurrentYear, 0x00, sizeof(szCurrentYear));
	sprintf(szCurrentYear, "20%02d", SetRTC.bYear);
	vdDebug_LogPrintf("Current year:szCurrentYear=[%s]", szCurrentYear);

	memset(szSysYear, 0x00, sizeof(szSysYear));
	inCTOSS_GetEnvDB("SYSYEAR", szSysYear);
	vdDebug_LogPrintf("Get ENV szSysYear=[%s] vs szCurrentYear=[%s]", szSysYear, szCurrentYear);

	if (strlen(szSysYear) <= 0 || atoi(szCurrentYear) < atoi(szSysYear))
	{
		vdDebug_LogPrintf("Update system clock...");
		put_env_char("SYSYEAR",szSysYear);

		memset(szDateTime, 0x00, sizeof(szDateTime));
		sprintf(szDateTime, "%02d|%02d|x|%02d|%02d|%02d|%s", SetRTC.bMonth, SetRTC.bDay, SetRTC.bHour, SetRTC.bMinute, SetRTC.bSecond, szSysYear);
		vdDebug_LogPrintf("Updated date=[%s]", szDateTime);
		usSetDateTime(szDateTime);

	}


	if (atoi(szCurrentYear) > atoi(szSysYear)){
		vdDebug_LogPrintf("Update system clock...");
		put_env_char("SYSYEAR",szCurrentYear);
	}

	// read again
	memset(szSysYear, 0x00, sizeof(szSysYear));
	inCTOSS_GetEnvDB("SYSYEAR", szSysYear);
	vdDebug_LogPrintf("Read again ENV szSysYear=[%s]", szSysYear);
	#if 0
	if (atoi(szCurrentYear) != atoi(szSysYear) || fWrite)
	{
		while(1)
	    {
			inDisplayMessageBoxWithButtonConfirmation(1,8,"This terrminal is outdated","and unable to function properly","please contact vendor for support","",MSG_TYPE_WARNING, BUTTON_TYPE_NONE_OK);
			CTOS_Beep();
			CTOS_Delay(500);
			continue;
	    }
