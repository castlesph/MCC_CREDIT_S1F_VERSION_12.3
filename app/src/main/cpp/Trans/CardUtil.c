...
void vdRemoveCard(void)
{
    ...
    while(1)
    {
        ...
        //if (linux_crttime >= linux_time1 + 2000)
        //{
            //break;
        //}
        ...
        if(bySC_status & d_MK_SC_PRESENT)
        {
            ...
            if (!fDisplayBox)
            {
//fix fo crash - remove this code, not being used anymore but causes crash
#if 0
...
short shAnalysisTrack2(BYTE *szTrack2, USHORT usTRack2Len) //Invalid card reading fix -- jzg
{
	...
	for(usIndex = 0; usIndex < usTRack2Len; usIndex++)
	{
		...
		if(byExpFlag)
		{
			...
			byExpFlag = 0;
			bySerCodeFlag = 1;
			//vdDebug_LogPrintf("srTransRec.szExpireDate");
		}
    ...
    for(usIndex = 0; usIndex < usTRack1Len; usIndex++)
    {
        ...
        if( byNameFlag == 1)
        {
		  ...
		  continue;
            	...
            	srTransRec.szCardholderName[usNameIndex] = szTrack1[usIndex];
				//vdDebug_LogPrintf("srTransRec.szCardholderName");
           	usNameIndex++;
	     }
        ...
        if(bySerCodeFlag)
        {
			...
			//vdDebug_LogPrintf("srTransRec.szServiceCode");
            return;
        }
    }
