...
*/
	...
	if (strTCT.fECR)
	{
         ...
         {
              if((srTransRec.HDTid!=MCC_HOST) && (strcmp(srTransRec.szECRRespCode,"00")!=0))
              	{
...
int inCTOS_MultiAPBatchSearch(int IPC_EVENT_ID)
{
	...
	vdDebug_LogPrintf("inDatabase_InvoiceNumSearch=inResult[%d], srTransRec.HDTid[%d]", inResult, srTransRec.HDTid);
    if(inResult == d_OK)
    {
    	// sidumili: selected credit app and record is wechat/alipay
    	if (inSettlementHost == 1 && byHoldTransType == VOID && (srTransRec.HDTid == WECHAT_HOST || srTransRec.HDTid == ALIPAY_HOST || srTransRec.HDTid == GCASH_HOST || srTransRec.HDTid == GRAB_HOST || srTransRec.HDTid == UPI_HOST))
    	{
		...
		if (inSettlementHost == 3 && byHoldTransType == VOID && (srTransRec.HDTid == MCC_HOST))
    	{
    		vdDisplayMessageBox(1, 8, "", "VOID NOT ALLOWED", "", MSG_TYPE_ERROR);
			CTOS_Beep();
	    	CTOS_Delay(1500);
...
int inCTOS_MultiSwitchApp(const char szAppName[100+1], int IPC_EVENT_ID)
{
	BYTE bret;
	BYTE outbuf[d_MAX_IPC_BUFFER];
	USHORT out_len = 0;
