...
//check if signal is OK
	...
	vdDebug_LogPrintf("saturn --inCTOSS_COMM_Initialize,chGetInit_Connect=[%d] inMode[%d]",chGetInit_Connect(), inMode);
    ...
    vdDebug_LogPrintf("saturn inCTOSS_COMM_Initialize %d - %d", usResult, bOutBuf[0]);
	if(d_OK == usResult)
    {
        //status
...
//test only
	...
	*usNetworkType = Sharls_COMData.usGPRSNetworkType;
	...
	vdDebug_LogPrintf("saturn ---wifi status[%d],WIFISSID=[%s], signal[%d]", Sharls_COMData.usWIFIType,Sharls_COMData.szWIFISSID, Sharls_COMData.inReserved2);
    ...
    if(d_OK == usResult)
    {
        ...
        if(bOutBuf[0] == IPC_COMM_SUCC){
			...
			vdDebug_LogPrintf("saturn return signal %d", fGPRSConnectOK);
            return inSignal;
        }	
        else{
			fGPRSConnectOK = FALSE;
			...
			vdDebug_LogPrintf("saturn return result 1 %d", fGPRSConnectOK);
            return usResult;
        }
    }
	else{
		...
		vdDebug_LogPrintf("saturn return resuelt 2 %d", fGPRSConnectOK);
		return usResult;
	}

	
	vdDebug_LogPrintf("saturn return resutl 3 %d", fGPRSConnectOK);
    return usResult;
}
...
int inCTOSS_SIMGetGPRSIP(void)
{
	...
	vdDebug_LogPrintf("saturn ---inCTOSS_SIMGetGPRSIP[%ld] %d",usResult, bOutBuf[0]);
	if(d_OK == usResult)
    {
        if(bOutBuf[0] == IPC_COMM_SUCC)
        {
...
int inCTOSS_SIMGetGPRSIPInfo(void)
{
	...
	vdDebug_LogPrintf("saturn ---inCTOSS_SIMGetGPRSIPInfo[%ld] %d",usResult, bOutBuf[0]);
	if(d_OK == usResult)
    {
        if(bOutBuf[0] == IPC_COMM_SUCC)
        {
