	...
	if(d_OK == usResult)
    {
		...
		if (memcmp(bOutBuf,SHARE_EMV_SUB_IPC,3) == 0)
		{
			...
			if (memcmp(&bOutBuf[3],SHARE_EMV_ONLINEPIN,1) == 0)
			{
				...
				if (srTransRec.byTransType == PREAUTH_VER || srTransRec.byTransType == SALE_OFFLINE)
				{
                    usResult = d_KMS2_GET_PIN_NULL_PIN;					
				}
				else
				{
			  ...
			  {		
				 ...
				 /* memset(szDisMsg,0,sizeof(szDisMsg));
				  ...
				  CTOS_Delay(700);*/
				  vdDisplayMessageBox(1, 8, "VERIFY PIN OK", "", "", MSG_TYPE_SUCCESS);
		                    CTOS_Beep();
		                    CTOS_Delay(700);
					 ...
					 if (memcmp(&bOutBuf[3],SHARE_EMV_SHOWVIRTUALPIN,1) == 0)
					  {
						  ...
						  usInLen = strlen(bInBuf);
						  vdDebug_LogPrintf("AAA - usInLen=[%d],str=[%s]",usInLen,bInBuf);
						  ...
						  while (1)
						  {
						  	if(d_OK == usResult)
						    {
								...
								if (memcmp(bOutBuf,SHARE_EMV_SUB_IPC,3) == 0)
								{
									  ...
									  {
										  vdDebug_LogPrintf("SHARE_EMV_GETPINDONE [%d], inPinDigit=%d", atoi(&bOutBuf[4]), inPinDigit);

										  usResult = OnGetPINDone();
                                          vdDebug_LogPrintf("OnGetPINDone usResult=%d", usResult);
                                          if ((strTCT.fOfflinePinbypass == 1) && (inPinDigit == 0))
                                          {
                                              vdDebug_LogPrintf("pin by passed");
                                          }
                                          else
                                          {
                                              if((inPinDigit) > 6 || (inPinDigit < 4))
                                              {
                                                  vdDebug_LogPrintf("Not in the range,inPinDigit=%d", inPinDigit);
                                                  strcpy(szDisMsg, " ");
                                                  strcat(szDisMsg, "|");
                                                  strcat(szDisMsg, "PIN digit need 4 ~ 6 digit");
...
//                                                  continue;
										  ...
										  if (d_OK == usResult)
										  {
										     if (memcmp(&bOutBuf[3],SHARE_EMV_ShOWPINDIGIT,1) == 0)
                                              {
                                                      vdDebug_LogPrintf("Renter PIN usOutLen=[%d],bOutBuf=[%s]",usOutLen,bOutBuf);
                                                    inReEnterPin = 1;
                                                    continue;
                                              }
										  		inReEnterPin = 0;
...
int inCTOSS_EMVSetFont(char *font)
{
	...
	strcpy(bInBuf,font);
	usInLen = strlen(bInBuf);
    usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_EMV.SHARLS_EMV", d_IPC_CMD_SETFONT, bInBuf, usInLen, bOutBuf, &usOutLen);
