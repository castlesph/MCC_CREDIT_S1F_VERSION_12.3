...
{    
	...
	if(strlen(szSerialNum) <= 0)
	    usGetSerialNumber(szSerialNum); //CTOS_GetFactorySN(szSerialNum);
	...
	vdDebug_LogPrintf("szSerialNum=[%s].tmpbuf=[%s].lenght[%d]",szSerialNum,tmpbuf, strlen(tmpbuf));
    ...
    if(ETHERNET_MODE == strTCT.inTMSComMode)
    {
       //CTOS_LCDTClearDisplay();
		...
		vdDebug_LogPrintf("-- TMS CONFIG [ETHERNET MODE]--");
    }
    else if(DIAL_UP_MODE == strTCT.inTMSComMode)
    {
       ...
       //CTOS_LCDTClearDisplay();
		...
		vdDebug_LogPrintf("-- TMS CONFIG [DIALUP MODE]--");
    }
    else if(GPRS_MODE == strTCT.inTMSComMode)
    {
		...
		*/
        ...
        vdDebug_LogPrintf("TMS COMM MODE -- GPRS_MODE");
        memset(&stgprs, 0x00, sizeof (CTMS_GPRSInfo));
        //bSIMSlot = 1;
		...
		vdDebug_LogPrintf("-- TMS CONFIG [GPRS MODE]--");
		...
		vdDebug_LogPrintf("stgprs.bSIMSlot[%d]", stgprs.bSIMSlot);
		vdDebug_LogPrintf("stgprs.ulSIMReadyTimeout[%d]", stgprs.ulSIMReadyTimeout);
       ...
       //CTOS_LCDTClearDisplay();
        ...
        //}
          //CTOS_LCDTClearDisplay();
		   ...
		   vdDebug_LogPrintf("-- TMS CONFIG [WIFI MODE]--");
...
int inCTOSS_SettlementCheckTMSDownloadRequest(void)
{
    ...
    else if(GPRS_MODE == strTCT.inTMSComMode)
    {
        ...
        //usResult=inCTOSS_COMM_Force_End();
        //vdDebug_LogPrintf("inCTOSS_COMM_Force_End: (%d)", usResult);	
    }
	else if(WIFI_MODE == strTCT.inTMSComMode)
    {
    ...
    vdDebug_LogPrintf("inCTOSS_TMSPreConfig");
    usResult = inCTOSS_TMSPreConfig(usComType, FALSE);
    if(usResult != d_OK);
        return d_NO;    
...
int inCTOSS_TMSDownloadRequest(void)
{
    ...
    usResult = inCTOSS_TMSPreConfig2(usComType, TRUE);
    if(usResult != d_OK)
        return d_NO;
	
    vdDebug_LogPrintf("inCTOSS_TMSPreConfig ret[%d] inTMSComMode[%d] usComType[%d]", usResult, strTCT.inTMSComMode, usComType);
//test only
/*
    setLCDPrint(8, DISPLAY_POSITION_LEFT, "TMS Download");
    CTOS_Delay(10000);
...
int inCTOSS_TMSDownloadRequestAfterSettle(void)
{
	...
	vdDebug_LogPrintf("--inCTOSS_TMSDownloadRequestAfterSettle--");
	...
	vdDebug_LogPrintf("fSettleAutoDL[%d]", strTCT.fSettleAutoDL);
	vdDebug_LogPrintf("inTMSComMode[%d]", strTCT.inTMSComMode);
	vdDebug_LogPrintf("usTMSGap[%d]", strTCT.usTMSGap);
    ...
    vdDebug_LogPrintf("inCTOSS_TMSPreConfig ret[%d] inTMSComMode[%d]", usResult, strTCT.inTMSComMode);
...
int inCTOS_CTMSUPDATE(void)
{
	...
	if (strlen(strTMSEX.szSerialNo) < SERIAL_NUM_LEN || atoi(strTMSEX.szSerialNo) <= 0)
	{
		...
		vdDisplayErrorMsg(1, 8, "Invalid serial number");
		return d_NO;
	}
	
#if 1
    vdDebug_LogPrintf("strCPT.inCommunicationMode=%d, strTCT.byRS232ECRPort=%d", strCPT.inCommunicationMode, strTCT.byRS232ECRPort);
	if(get_env_int("#CRADLE") == 1 || strTCT.byRS232ECRPort == 11)
	{
		#if 0
		...
		#endif
		{
            vdDebug_LogPrintf("1.inCTOS_CTMSUPDATE");
            vdSetInit_Connect(1);
	...
	else
	{
        ...
        vdDebug_LogPrintf("2.inCTOS_CTMSUPDATE");
...
int inCTOS_CTMSUPDATE_BackGround(void)
{
    ...
    {
        //inDisplayMessageBoxWithButtonConfirmation(1,8,strTMSEX.szSerialNo,"Invalid serial number","","",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);
        vdDisplayMessageBox(1, 8, strTMSEX.szSerialNo, "Invalid serial number", "", MSG_TYPE_WARNING);
        CTOS_Beep();
        CTOS_Delay(1500);
        CTOS_Beep();
    ...
    vdDebug_LogPrintf("inCTOS_ChkBatchEmpty_AllHosts, inRet=[%d]", inRet);
    if (d_OK != inRet) {
        vdDebug_LogPrintf("saturn =====inCTOS_CTMSUPDATE_BackGround Batch not Empty=====");
        vdCTOS_TransEndReset();
        return inRet;
    }
    ...
    memset(szCurrDate, 0x00, sizeof(szCurrDate));
    sprintf(szCurrDate,"%02d%02d%02d", SetRTC.bYear, SetRTC.bMonth, SetRTC.bDay);
    vdDebug_LogPrintf("AAA>> inCTOS_CTMSUPDATE_BackGround szCurrDate[%s]", szCurrDate);
    put_env_char("AUTODLDATE",szCurrDate);
    ...
    vdDebug_LogPrintf("saturn End.=====inCTOS_CTMSUPDATE_BackGround=====");
    vdDisplayMessageBox(1, 8, "Processing", "", MSG_PLS_WAIT, MSG_TYPE_PROCESS);
    CTOS_Delay(500);
    ...
    vdDebug_LogPrintf("inCallJAVA_CTMSUPDATEBackGround, inRet=[%d]", inRet);
