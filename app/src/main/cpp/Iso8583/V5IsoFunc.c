...
int inProcessAdviceTrans(TRANS_DATA_TABLE *srTransPara, int inAdvCnt)
{
    ...
    while(1)
    {
        ...
        if(inResult == ST_SUCCESS)
        {
			...
			vdDebug_LogPrintf("srAdvTransTable.byTransType: %d", srAdvTransTable.byTransType);

			if(srAdvTransTable.byTransType == PREAUTH_VER)
                srAdvTransTable.byPackType=PREAUTH_COMP;
...
#if 0
{
    ...
    vdDebug_LogPrintf("inPackSendAndUnPackData srTransRec.byTransType: %d, inTransCode: %d", srTransRec.byTransType, inTransCode);
...
#if 0
{
    ...
    if((inTransCode != BATCH_UPLOAD) && (inTransCode != REVERSAL) && (inTransCode != SETTLE) && (inTransCode != CLS_BATCH))
    {
        ...
        if ((srTransPara->byEntryMode == CARD_ENTRY_ICC || 
            ...
            || srTransPara->byTransType == PREAUTH_VOID)) // rtr 05252015 issue
        {

            vdDebug_LogPrintf("Transcation set DE55 T5F34_len[%d], srTransPara->byTransType:%d", srTransPara->stEMVinfo.T5F34_len, srTransPara->byTransType);
	...
	if(srTransPara->byTransType == PREAUTH_COMP_VOID)
	{
		vdCTOSS_SetBitMapOff(inBitMap, 26);
		vdCTOSS_SetBitMapOff(inBitMap, 52);	
...
/*
    if (srTransPara->byCeillingAmt == VS_TRUE)
    {
        ...
        vdDebug_LogPrintf("1.vdModifyBitMapFunc srTransPara->byTransType:%d",srTransPara->byTransType);
        
        if(srTransPara->byTransType == SALE_OFFLINE || srTransPara->byTransType == SALE_TIP)
        {
        }
		else
		{
...
/*
    //EMV - Void Offine Sale
    if(srTransPara->byEntryMode == CARD_ENTRY_ICC && srTransPara->byPackType == VOID && srTransPara->byOrgTransType == SALE_OFFLINE)
    {
		vdCTOSS_SetBitMapOn(inBitMap, 55);
...
*/
	vdDebug_LogPrintf("srTransPara->byEntryMode:%d, srTransPara->byPackType:%d, srTransPara->byOrgTransType:%d", srTransPara->byEntryMode, srTransPara->byPackType, srTransPara->byOrgTransType);
		 ...
		 else if(strTCT.fTLESec== TRUE && strHDT.fHostTLESec== TRUE && (srTransPara->byTransType != DCC_RATE_REQUEST && srTransPara->byTransType != DCC_MERCH_RATE_REQ))
		 {
			...
			// Parse ISO Send
			 ...
			 {	
				...
				inPrintISOPacket("TX" , uszPrntSendData, inPrntLen);
			 }
			 ...
			 if(inResult == d_OK && fISOLog)
			 {
				...
				inPrint("*TLE-ENCRYPTED PACKET*");
				inLogISOPacket("TX" , uszPrntSendData, inPrntLen);
			 }
     ...
     if (inMsgid!=ST_SUCCESS)
     {
          if(0 == strlen(szResponseCode) )
          {
              // inMSGResponseCodeRead(szResponseCode, szResponseCode2, inMsgid, 1);
               if(0 != strlen(szResponseCode) )
               {		
			   	    if(get_env_int("RR_ERR_RESP") == 0 && srTransRec.byTransType == DCC_RATE_REQUEST)
			   	    {
			   	    }
					else
					{
...
int inPackIsoFunc03(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
    ...
    else if(srTransPara->byTransType == PREAUTH_COMP_VOID)
    {
        if(srTransPara->byPackType == REVERSAL)
			uszSendData[0]=0x02;
    }
...
int inPackIsoFunc22(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
    ...
    vdDebug_LogPrintf("inPackIsoFunc22 byTransType:%d", srTransPara->byTransType);
...
//#00169
	...
	else if(srTransPara->byPackType == PREAUTH_COMP || srTransPara->byPackType == PREAUTH_COMP_VOID
		|| (srTransPara->byTransType == PREAUTH_COMP_VOID && srTransPara->byPackType == REVERSAL))
	{
	     wub_str_2_hex("06", (char *)&uszSendData[inDataCnt], 2);
	    inDataCnt += 1;
	    return (inDataCnt);
...
int inPackIsoFunc60(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
    ...
    else
    {
        ...
        else
        {
			...
			else if(srTransPara->byPackType == PREAUTH_COMP_VOID)
			{
				wub_hex_2_str(srTransPara->szBaseAmount,szPacket,6);
				//wub_hex_2_str(srTransPara->szOrgAmount,szPacket,6);
			}
    ...
    else
    {
		...
		else
		{
			...
			if (strTCT.fTSNEnable == TRUE)
			{
                ...
                vdDebug_LogPrintf("srTransPara->byPackType[%d], srTransPara->byEntryMode[%d]", srTransPara->byPackType, srTransPara->byEntryMode);
				inResult=inIITRead(srTransPara->IITid);
				
				vdDebug_LogPrintf("inIITRead inResult[%d], strIIT.fTag20[%d]", inResult, strIIT.fTag20);
				
				if(srTransPara->byPackType == BATCH_UPLOAD || srTransPara->byPackType == SEND_ADVICE  
				|| srTransPara->byPackType == SALE_OFFLINE || srTransPara->byPackType == PREAUTH_COMP
				|| srTransPara->byPackType == SALE_TIP || srTransPara->byPackType == PREAUTH_COMP_VOID
				|| (srTransPara->byPackType == VOID && srTransPara->byOrgTransType == SALE_OFFLINE && srTransPara->byUploaded == TRUE)) // for testing, tag 20 should be included in DE60.
				{
                    if(srTransPara->byEntryMode == CARD_ENTRY_ICC || srTransPara->byEntryMode == CARD_ENTRY_WAVE)
                    {
						inIITRead(srTransPara->IITid);
						vdDebug_LogPrintf("2.strIIT.fTag20[%d]", strIIT.fTag20);
...
{ 
...

	{
        ...
        #if 1
		DebugAddHEX("1.T9F34", srTransPara->stEMVinfo.T9F34, 3);
		inWaveGetValue("DF8126", "MASTERCARD");
		memset(szAmt, 0, sizeof(szAmt));
		wub_hex_2_str(srTransPara->szTotalAmount, szAmt, AMT_BCD_SIZE);
		if(atol(szAmt) < atol(srTransRec.szWaveValue))
		{
			srTransPara->stEMVinfo.T9F34[0]=0x1F;
		}
        else
        {
            if(srTransPara->fEMVPIN == FALSE)
                srTransPara->stEMVinfo.T9F34[0]=0x1E;	
        }
		DebugAddHEX("2.T9F34", srTransPara->stEMVinfo.T9F34, 3);
