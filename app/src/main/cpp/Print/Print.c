...
extern unsigned char gblszMarginRatePercentage[10+1], gblszExchangeRate[12+1], gblszDCCCurrencySymbol[3+1],  gblszCurrencyCode[3+1], gblszForeignAmount[20+1],  gblszLocalAmount[20+1], gblszCurrForeignAmount[20+1],
...
USHORT ushCTOS_PrintFooter(int page)
{
	...
	vdDebug_LogPrintf("fEMVPINBYPASS[%d]", srTransRec.fEMVPINBYPASS);
     ...
     else if(page == MERCHANT_COPY_RECEIPT || page == BANK_COPY_RECEIPT)
     {
          ...
          if((srTransRec.byTransType == VOID && (srTransRec.byTransType == PREAUTH_COMP || srTransRec.byTransType == PREAUTH_VER)) || srTransRec.byTransType == PREAUTH_COMP_VOID)
          {
		  	vdCTOS_PrinterFline(1);
...
USHORT ushCTOS_printAll(int page) {
	...
	*/
	...
	if (strTCT.feReceiptEnable == 1) {
        ...
        vdCTOSS_PrinterBMPPicEx(0, 0, szLogoPath);
		vdCTOSS_PrinterBMPPicEx(0, 0, "/data/data/pub/Print_BMP.bmp");  //TINE:  temporarily remove for debugging purposes on on-screen-signing functionality
        vdCTOSS_PrinterEnd();
        //vdSetPrintThreadStatus(0);
...
//    	CTOS_PrinterPutString("vdCTOS_PrintSummaryReport");
	...
	shHostIndex = inCTOS_SelectHostSetting();
	if (shHostIndex == -1 || shHostIndex == -2)
		return;
	...
	if (strTCT.fDCCTerminal==TRUE)
	{
		...
		vdDebug_LogPrintf("saturn 2 AAA - srAccumRec.stBankTotal[0].stDCCTotal[%d].usOffSaleDCCCount[%d]", j,srAccumRec.stBankTotal[0].stDCCTotal[j].usOffSaleDCCCount);
                   //fix for crash
				   memset(szStr, 0x00, sizeof(szStr));
                   strcpy(szStr, strCST.szCurSymbol);
				   vdDebug_LogPrintf("saturn 2 before inPrint");
                   inPrint(szStr);
...
USHORT ushCTOS_printErrorReceipt(void)
{
    ...
    if(get_env_int("RR_ERR_RCPT") == 0 && srTransRec.byTransType == DCC_RATE_REQUEST)
		return (d_OK);
...
void vdCTOS_PrintAccumeByHostAndCard (int inReportType, STRUCT_TOTAL Totals)
{
    ...
    memset(szTemp, 0x00, d_LINE_SIZE);
    memset(szFormattedAmount, 0x00, d_LINE_SIZE);
    sprintf(szTemp,"%ld", ulVoidSaleTotalAmount+ulVoidCompTotalAmount); //included Void Completion total amount
    ...
    //vdCTOS_FormatAmount("NNN,NNN,NNn.nn", szTemp,szFormattedAmount);
    sprintf(szStr,"VOID            %3d       %s", usVoidSaleCount+usVoidCompCount,strCST.szCurSymbol); //included Void Completion total count
    inPrintLeftRight(szStr, szFormattedAmount, 46);
...
//    	CTOS_PrinterPutString("ushCTOS_ReprintLastSettleReport");
   ...
   //CTOS_LCDTClearDisplay(); // fix on issue #00106 Error message must not appear on Host Selection after selecting host
	...
	DebugAddHEX("srAccumRec.szYear", srAccumRec.szYear, 2);
	DebugAddHEX("1.srTransRec.szYear", srTransRec.szYear, 2);
...

{
	...
	// Capture report -- sidumili
	vdDebug_LogPrintf("Capturing erm report... strGBLVar.fGBLvConfirmPrint:%d", strGBLVar.fGBLvConfirmPrint);
	vdDeleteBMP();
	vdCTOSS_PrinterEnd_CConvert2BMP("/data/data/pub/Print_BMP.bmp");
	CTOS_KBDBufFlush();
	inCTOSS_ERM_Form_Receipt(0);
...
//end
  ...
  else
  {
	...
	memset(szStr, 0x00, d_LINE_SIZE);
	if(strlen(strCPT.szPriTxnHostIP) >= 32)
	    sprintf(szStr, "PRI IP  : %s", (char *)strCPT.szPriTxnHostIP);
	else
		sprintf(szStr, "PRI IP      : %s", (char *)strCPT.szPriTxnHostIP);
	inPrint(szStr);
	...
	memset(szStr, 0x00, d_LINE_SIZE);
	if(strlen(strCPT.szSecTxnHostIP) >= 32)
	    sprintf(szStr, "SEC IP  : %s", (char *)strCPT.szSecTxnHostIP);
	else
		sprintf(szStr, "SEC IP      : %s", (char *)strCPT.szSecTxnHostIP);
	inPrint(szStr);
	...
	memset(szStr, 0x00, d_LINE_SIZE);
	if(strlen(strCPT.szPriSettlementHostIP) >= 32)
	...
	memset(szStr, 0x00, d_LINE_SIZE);
	sprintf(szStr, "PRI SET PORT: %04d", strCPT.inPriSettlementHostPort);
	inPrint(szStr);

	memset(szStr, 0x00, d_LINE_SIZE);
	sprintf(szStr, "SEC SET PORT: %04d", strCPT.inSecSettlementHostPort);
	inPrint(szStr);
...
void vdPrintCRC(void)
{
	...
	for(inNum =1 ;inNum <= inNumOfHost; inNum++)
	{
		if(inHDTRead(inNum) == d_OK)
		{
			memset(szAPName, 0x00, sizeof(szAPName));
			//memset(szCRC, 0x00, sizeof(szCRC));
			if (strcmp("com.Source.S1_MCC.MCC", strHDT.szAPName)==0)
			{				
                strcpy(szAPName, "MCC ONE APP ");
                memset(szCRC, 0x00, sizeof(szCRC));
                strcpy(szCRC, "143ADD579");
			...
			{				
                ...
                fQRPrnt  = TRUE;
                memset(szCRC, 0x00, sizeof(szCRC));
                strcpy(szCRC, "68D33ABF1");
                
                //getAppPackageInfo(strHDT.szAPName, szCRC); // comment for HC
                memset(szPrintCRC, 0x00, sizeof(szPrintCRC));
                sprintf(szPrintCRC, "%s: %s", szAPName, szCRC);
                inPrint(szPrintCRC);
			}
...
USHORT ushCTOS_DCCPrintBody(int page)
{
    ...
    if(CUSTOMER_COPY_RECEIPT == page)
    {
        ...
        vdPrintECRMessage(); // Mercury Requirements -- sidumili
		        ...
		        vdDebug_LogPrintf("saturn 2 dcc crash test after ecrprint message 2");
        ...
        memset(szTemp1, ' ', sizeof(szTemp1));
        memset(szTemp3, ' ', sizeof(szTemp3));
        memset(szAmtBuf, ' ', sizeof(szAmtBuf));
		//end of fix
    ...
    else if(MERCHANT_COPY_RECEIPT == page || BANK_COPY_RECEIPT== page)
    {
        ...
        vdDebug_LogPrintf("saturn 2 dcc crash test after ecrprint message");
		...
		vdDebug_LogPrintf("saturn 2 dcc crash test 8");
...
void vdCTOS_PrintAccumeByDCCCurrency (int inReportType, STRUCT_TOTAL Totals)
{
    ...
    char szSaleDCCTotalAmount[12+1];
	char szRefundTotalDCCAmount[12+1];
	char szTipTotalDCCAmount[12+1];
	char szVoidSaleTotalDCCAmount[12+1];
	char szOffSaleTotalDCCAmount[12+1];
	char szCompTotalDCCAmount[12+1];

	memset(szSaleDCCTotalAmount, 0, sizeof(szSaleDCCTotalAmount));
	memset(szRefundTotalDCCAmount, 0, sizeof(szRefundTotalDCCAmount));
	memset(szTipTotalDCCAmount, 0, sizeof(szTipTotalDCCAmount));
	memset(szVoidSaleTotalDCCAmount, 0, sizeof(szVoidSaleTotalDCCAmount));
	memset(szOffSaleTotalDCCAmount, 0, sizeof(szOffSaleTotalDCCAmount));
	memset(szCompTotalDCCAmount, 0, sizeof(szCompTotalDCCAmount));	

    wub_hex_2_str(Totals.szSaleDCCTotalAmount, szSaleDCCTotalAmount, 6);
	wub_hex_2_str(Totals.szRefundTotalDCCAmount, szRefundTotalDCCAmount, 6);
	wub_hex_2_str(Totals.szTipTotalDCCAmount, szTipTotalDCCAmount, 6);
	wub_hex_2_str(Totals.szVoidSaleTotalDCCAmount, szVoidSaleTotalDCCAmount, 6);
	wub_hex_2_str(Totals.szOffSaleTotalDCCAmount, szOffSaleTotalDCCAmount, 6);
	wub_hex_2_str(Totals.szCompTotalDCCAmount, szCompTotalDCCAmount, 6);
   ...
   vdSetGolbFontAttrib(d_FONT_16x16, NORMAL_SIZE, DOUBLE_SIZE, 0, 0);
    ...
    vdCTOS_FormatAmount(strCST.szAmountFormat, szSaleDCCTotalAmount, szFormattedAmount);
    ...
    vdCTOS_FormatAmount(strCST.szAmountFormat, szCompTotalDCCAmount,szFormattedAmount);
    ...
    vdCTOS_FormatAmount(strCST.szAmountFormat, szVoidSaleTotalDCCAmount,&szFormattedAmount[1]);
    szFormattedAmount[0]='-'; 
    ...
    vdCTOS_FormatAmount(strCST.szAmountFormat, szRefundTotalDCCAmount,&szFormattedAmount[1]);
    szFormattedAmount[0]='-'; 
   ...
   {
        vdCTOS_FormatAmount("NNN,NNN,NNn.nn", szTipTotalDCCAmount, szFormattedAmount);
  ...
  {
       vdCTOS_FormatAmount(strCST.szAmountFormat, szTipTotalDCCAmount,szFormattedAmount);
       sprintf(szStr,"TIPS                     %s",strCST.szCurSymbol);
       inPrintLeftRight(szStr, szFormattedAmount, 46);
  }
    //DCC OFFLINE SALE
    ...
    vdCTOS_FormatAmount(strCST.szAmountFormat, szOffSaleTotalDCCAmount,szFormattedAmount);
