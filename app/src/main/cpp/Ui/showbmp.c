...
void displayAppbmpDataEx(unsigned int x,unsigned int y, const char *filename, BOOL fPublic)
{
	...
	vdDebug_LogPrintf("--displayAppbmpDataEx--");
	vdDebug_LogPrintf("filename[%s]", filename);
	vdDebug_LogPrintf("fPublic[%d]", fPublic);
	
    memset(szBmpfile, 0x00, sizeof(szBmpfile));
	if(fPublic == TRUE)
        sprintf(szBmpfile,"%s%s", PUBLIC_PATH,filename);		
	else	
        sprintf(szBmpfile,"%s%s", LOCAL_PATH,filename);
...
//    CTOS_LCDBackGndColor(RGB(77,153,211));
    ...
    CTOS_LCDForeGndColor(RGB(25,25,112)); /*midnight blue*/
...
void vdThreadDisplayPrintBmp(unsigned int x,unsigned int y, const char *path1, const char *path2, const char *path3, const char *path4, const char *path5)
{
    ...
    vdDebug_LogPrintf("vdThreadDisplayPrintBmp.end");
