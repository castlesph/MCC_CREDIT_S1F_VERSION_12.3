...
{    
	...
	//else if(srTransRec.fDebit == TRUE && strcmp(srTransRec.szCardLable, "BANCNET") == 0)//#00023
        ...
        case VOID:
            ...
            else if(srTransRec.byOrgTransType == PREAUTH_COMP || srTransRec.byOrgTransType == PREAUTH_VER)
                strcpy(szTitle, "VOID PRE COMP");		
            else
                strcpy(szTitle, "VOID");
            break;
...
void vdDisplayTxnFinishUI(void)
{
    ...
    else
    {
        ...
        //setLCDPrint(5, DISPLAY_POSITION_CENTER, "Transaction");
        //setLCDPrint(6, DISPLAY_POSITION_CENTER, "Approved");
        //setLCDPrint(7, DISPLAY_POSITION_CENTER, srTransRec.szAuthCode);  

		#if 0
		//TINE:  android display
		...
		strcat(szDisMsg, "|");
		strcat(szDisMsg, "TRANSACTION APPROVED");
...
void vdDisplayErrorMsg(int inColumn, int inRow,  char *msg)
{
    ...
    if ((strTCT.byTerminalType % 2) == 0)
      ...
      usCTOSS_LCDDisplay(szDisMsg);
       ...
       CTOS_Beep();
       if (strcmp(msg, "PLEASE SEE PHONE")==0)
          CTOS_Delay(300);	
     else
         CTOS_Delay(1500);	
...
void vdDisplayErrorMsg2(int inColumn, int inRow,  char *msg, char *msg2, int msgType)
{
	...
	vdDebug_LogPrintf("saturn display msg %s", szDisMsg);
    usCTOSS_LCDDisplay(szDisMsg);
	#else
     vdDisplayMessageBox(1, 8, msg, msg2, "", msgType);
     CTOS_Beep();
     if((strcmp(msg, "CTLS ERROR PLEASE")==0) || (strcmp(msg, "CTLS READ CARD ERROR")==0))
             CTOS_Delay(300);	
     else
              CTOS_Delay(1500);	
...
void vdDisplayErrorMsg3(int inColumn, int inRow,  char *msg, char *msg2, char *msg3)
{
    ...
    //strcat(szDisMsg, msg);
     ...
     CTOS_Beep();
     if (strcmp(msg, "CTLS TRANSACTION")==0)
          CTOS_Delay(300);	
     else
         CTOS_Delay(1500);	
     //CTOS_Beep();
...
int inDisplayMessageBoxWithButtonReboot(int inColumn, int inRow,  char *msg, char *msg2, char *msg3, int msgType)
{
	BYTE szDisMsg[200 + 1] = {0};
	BYTE szMsgType[40 + 1] = {0};
	int inRet = d_NO;

	vdDebug_LogPrintf("--inDisplayMessageBoxWithReboot--");
	...
	inRet = usCTOSS_ConfirmReboot(szDisMsg);
