...
*/
        if(CreatedCtmsObj) {
            ctCtms = MccApplication.getCtCtmsObj();
            Log.i(TAG, "CreatedCtmsObj: " + (ctCtms == null));
                    ...
                    if((batteryRemain > 35) || isCounterTopTerminal)//S1KC terminal cap batteryRemain is 0
                    {
	                    ...
	                    if(!backUpDbFlag) {
	                        ...
	                        new Thread() {
	                            ...
	                            public void run() {
	                                try {
	                                    Log.i(TAG, "inCTOSS_BackupDataScript: start");
	                                    inCTOSS_BackupDataScript();
	                                    Log.i(TAG, "inCTOSS_BackupDataScript: end");
	                                } catch (Exception e) {
	                    ...
	                    Log.i(TAG, "After execute inCTOSS_BackupDataScript: sleep start");
	                    long inTBDSleep = inCTOSS_GetEnvInt("TBDSLEEP");
	                    if(inTBDSleep <= 0)
                            inTBDSleep=10000;

	                    Log.i(TAG, "inTBDSleep="+inTBDSleep);
	                    try {
	                        Thread.sleep(inTBDSleep);
	                    } catch (InterruptedException e) {
	                        e.printStackTrace();
	                    }
	                    Log.i(TAG, "After execute inCTOSS_BackupDataScript: sleep end");
...
/*            new Handler().postDelayed(new Runnable() {
        ...
        public void getTerminalInfoCallback(int i) {
            ...
            vdUpdateDownloadStatus("Upload app and FW information...");
        }
...
//                    inCTOSS_SetDownloadedNotFinishedFlag(1);//20210127 for CTMS
                    ...
                    String pFreeTypeUpdateList = ctCtms.getFreeTypeUpdateList();
                    Log.i(TAG, "pFreeTypeUpdateList="+pFreeTypeUpdateList);
                    // terminal reboot when has an update -- sidumili
                    // totalNum -> file count, if > 0 then included application during download
                    ...
                    if (inCTMSPRMSize != inDLFullSize || totalNum > 0 || inCTMSForceReboot > 0)
                    {
        ...
        public void freeTypeDownloadCallback(int i, DownloadInfo downloadInfo) {
            Log.i(TAG, "freeTypeDownloadCallback: run");
            /*Start timer*/
            if(isCounterRunning ){
                isCounterRunning = true;
                //getTimerCancel();
            ...
            Log.i("free-CTMS-cb ", "freeTypeDownloadCallback: " + downloadInfo.fileName + " DL size:" + downloadInfo.downloadSize + " full size: " + downloadInfo.fullSize + " totalDownloadNumber: " + downloadInfo.totalDownloadNumber + " downloadIndex: " + downloadInfo.downloadIndex);
            int progress = (int) ((((float) downloadInfo.downloadSize) / ((float) downloadInfo.fullSize)) * 100);
            Log.i(TAG, "progress: " + progress);
        ...
        public void freeTypeInstallCallback(int i, PackageInfo packageInfo) {
            Log.i(TAG, "freeTypeInstallCallback: run");
            /*Start timer*/
            if(isCounterRunning ){
                isCounterRunning = true;
                //getTimerCancel();
